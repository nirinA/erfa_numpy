...
_erfa_apco13(PyObject *self, PyObject *args)
{
    ...
    for (i=0;i<dims[0];i++) {
        j = eraApco13(utc1[i], utc2[i], dut1[i], elong[i],
...
_erfa_atci13(PyObject *self, PyObject *args)
{
    ...
    adate1 = PyArray_FROM_OTF(pydate1, NPY_DOUBLE, NPY_ARRAY_IN_ARRAY);
    adate2 = PyArray_FROM_OTF(pydate2, NPY_DOUBLE, NPY_ARRAY_IN_ARRAY);
    if (arc == NULL || adc == NULL || apr == NULL ||
        apd == NULL || apx == NULL || arv == NULL ||
...
_erfa_atciq(PyObject *self, PyObject *args)
{
    ...
    for (i=0;i<dims[0];i++) {
        ...
        Py_DECREF(a);
        eraAtciq(rc[i], dc[i], pr[i], pd[i], px[i], rv[i], &astrom, &ri[i], &di[i]);
    }
    Py_DECREF(arc);
    Py_DECREF(adc);
    Py_DECREF(apr);
...
static PyObject *
_erfa_atciqn(PyObject *self, PyObject *args)
{
    double *rc, *dc, *pr, *pd, *px, *rv;
    double *ri, *di;
    PyObject *pyrc, *pydc, *pypr, *pypd, *pypx, *pyrv;
...
_erfa_atio13(PyObject *self, PyObject *args)
{
    ...
    PyObject *pyri, *pydi, *pyutc1, *pyutc2, *pydut1, *pyelong, *pyphi, *pyhm, *pyxp, *pyyp, *pyphpa, *pytc, *pyrh, *pywl;
    PyObject *ari, *adi, *autc1, *autc2, *adut1, *aelong, *aphi, *ahm, *axp, *ayp, *aphpa, *atc, *arh, *awl;
    PyArrayObject *pyaob = NULL,  *pyzob = NULL, *pyhob = NULL, *pydob = NULL, *pyrob = NULL;
    PyArray_Descr * dsc;
    dsc = PyArray_DescrFromType(NPY_DOUBLE);
    npy_intp *dims;
    int ndim, i;
    if (!PyArg_ParseTuple(args, "O!O!O!O!O!O!O!O!O!O!O!O!O!O!",
                                 &PyArray_Type, &pyri,
                                 &PyArray_Type, &pydi,
                                 &PyArray_Type, &pyutc1,
                                 &PyArray_Type, &pyutc2,
    ...
    arh = PyArray_FROM_OTF(pyrh, NPY_DOUBLE, NPY_ARRAY_IN_ARRAY);
    awl = PyArray_FROM_OTF(pywl, NPY_DOUBLE, NPY_ARRAY_IN_ARRAY);
    if (ari == NULL || adi == NULL || autc1 == NULL || autc2 == NULL ||
        adut1 == NULL || aelong == NULL || aphi == NULL || ahm == NULL ||
        axp == NULL || ayp == NULL || aphpa == NULL || atc == NULL ||
        arh == NULL || awl == NULL) {
        goto fail;
    ...
    for (i=0;i<dims[0];i++) {
        j = eraAtio13(ri[i], di[i], utc1[i], utc2[i], dut1[i],
                      elong[i], phi[i], hm[i], xp[i], yp[i],
                      phpa[i], tc[i], rh[i], wl[i],
                      &aob[i], &zob[i], &hob[i], &dob[i], &rob[i]);
        if (j == +1) {
            PyErr_SetString(_erfaError, "doubious year");
            goto fail;
        }
...
_erfa_atoc13(PyObject *self, PyObject *args)
{
    ...
    double *rc, *dc;
    PyObject *pyob1, *pyob2, *pyutc1, *pyutc2, *pydut1, *pyelong, *pyphi, *pyhm, *pyxp, *pyyp, *pyphpa, *pytc, *pyrh, *pywl;
    PyObject *aob1, *aob2, *autc1, *autc2, *adut1, *aelong, *aphi, *ahm, *axp, *ayp, *aphpa, *atc, *arh, *awl;
    PyArrayObject *pyrc = NULL, *pydc = NULL;
    PyArray_Descr * dsc;
    dsc = PyArray_DescrFromType(NPY_DOUBLE);
    npy_intp *dims;
    int ndim, i;
    if (!PyArg_ParseTuple(args, "sO!O!O!O!O!O!O!O!O!O!O!O!O!O!",
                                 &type,
                                 &PyArray_Type, &pyob1,
                                 &PyArray_Type, &pyob2,
                                 &PyArray_Type, &pyutc1,
    ...
    arh = PyArray_FROM_OTF(pyrh, NPY_DOUBLE, NPY_ARRAY_IN_ARRAY);
    awl = PyArray_FROM_OTF(pywl, NPY_DOUBLE, NPY_ARRAY_IN_ARRAY);
    if (aob1 == NULL || aob2 == NULL || autc1 == NULL ||
        autc2 == NULL || adut1 == NULL || aelong == NULL ||
        aphi == NULL || ahm == NULL || axp == NULL ||
        ayp == NULL || aphpa == NULL || atc == NULL ||
        arh == NULL || awl == NULL) {
        goto fail;
    }
    ...
    for (i=0;i<dims[0];i++) {
        if (strcmp("R", type) == 0 || strcmp("H", type) == 0 || strcmp("A", type) == 0) {
            j = eraAtoc13(type, ob1[i], ob2[i], utc1[i], utc2[i], dut1[i],
                          elong[i], phi[i], hm[i], xp[i], yp[i], phpa[i],
                          tc[i], rh[i], wl[i],
                          &rc[i], &dc[i]);
        }
        else {
...
_erfa_atoi13(PyObject *self, PyObject *args)
{
    ...
    for (i=0;i<dims[0];i++) {
        if (strcmp("R", type) == 0 || strcmp("H", type) == 0 || strcmp("A", type) == 0) {
            j = eraAtoi13(type, ob1[i], ob2[i], utc1[i], utc2[i], dut1[i],
                          elong[i], phi[i], hm[i], xp[i], yp[i], phpa[i],
                          tc[i], rh[i], wl[i],
                          &ri[i], &di[i]);
        }
...
_erfa_refco(PyObject *self, PyObject *args)
{
    ...
    PyObject *pyphpa, *pytc, *pyrh, *pywl;
    PyObject *aphpa, *atc, *arh, *awl;
    PyArrayObject *pyrefa = NULL,  *pyrefb = NULL;
    PyArray_Descr * dsc;
    dsc = PyArray_DescrFromType(NPY_DOUBLE);
    npy_intp *dims;
    int ndim, i;
    ...
    arh = PyArray_FROM_OTF(pyrh, NPY_DOUBLE, NPY_ARRAY_IN_ARRAY);
    awl = PyArray_FROM_OTF(pywl, NPY_DOUBLE, NPY_ARRAY_IN_ARRAY);
    if (aphpa == NULL || atc == NULL || arh == NULL || awl == NULL) {
        goto fail;
    }
    ndim = PyArray_NDIM(aphpa);
    if (!ndim) {
        PyErr_SetString(_erfaError, "argument is ndarray of length 0");
        goto fail;
    }
    dims = PyArray_DIMS(aphpa);
    if (dims[0] != PyArray_DIMS(atc)[0] ||
        dims[0] != PyArray_DIMS(arh)[0] || dims[0] != PyArray_DIMS(awl)[0]) {
        PyErr_SetString(_erfaError, "arguments have incompatible shape ");
        goto fail;
    }    
    ...
    rh = (double *)PyArray_DATA(arh);
    wl = (double *)PyArray_DATA(awl);
    pyrefa = (PyArrayObject *) PyArray_Zeros(ndim, dims, dsc, 0);
    pyrefb = (PyArrayObject *) PyArray_Zeros(ndim, dims, dsc, 0);
    if (NULL == pyrefa || NULL == pyrefb) goto fail;
    refa = (double *)PyArray_DATA(pyrefa);
    refb = (double *)PyArray_DATA(pyrefb);

    for (i=0;i<dims[0];i++) {
        eraRefco(phpa[i], tc[i], rh[i], wl[i], &refa[i], &refb[i]);
    }
    Py_DECREF(aphpa);
    Py_DECREF(atc);
    Py_DECREF(arh);
    ...
    Py_INCREF(pyrefa);
    Py_INCREF(pyrefb);
    return Py_BuildValue("OO", pyrefa, pyrefb);
...
{
    double *theta, *phi, *c;
    PyObject *pytheta, *pyphi;
    PyObject *atheta, *aphi;
    ...
    theta = (double *)PyArray_DATA(atheta);
    phi = (double *)PyArray_DATA(aphi);
    pyc = (PyArrayObject *) PyArray_Zeros(2, dim_out, dsc, 0);
    if (NULL == pyc) goto fail;
    c = (double *)PyArray_DATA(pyc);

    for (i=0;i<dims[0];i++) {
        eraS2c(theta[i], phi[i], &c[i*3]);
    }
    Py_DECREF(atheta);
    Py_DECREF(aphi);
    Py_INCREF(pyc);
    return PyArray_Return(pyc);
